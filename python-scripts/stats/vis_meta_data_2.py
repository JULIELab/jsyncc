...
def get_colors(n):
    return [hsv_to_rgb(((0.85/n) * (i+0.5), 1, 1)) for i in range(n)]
...
jsyncc_counts = jsyncc_counts.sort_index()
table_out = jsyncc_counts.transpose().sort_index()

print(table_out)
table_out.round(0).astype(str).to_latex(output + os.sep + 'jsyncc_topic_counts.tex')
table_out.to_excel(output + os.sep + 'jsyncc_topic_counts.xlsx')
...
plt.tight_layout()
plt.savefig(output + os.sep + 'jsyncc_topics_per_document_type.png', dpi=300)
plt.savefig(output + os.sep + 'jsyncc_topics_per_document_type.svg', dpi=300)
plt.close()
