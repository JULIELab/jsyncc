...
public class CasesOphthalmology
{
			...
			for (int i = 0; i < lines.length; i++)
			{
				...
				if (line.endsWith("Klinischer Fall"))
				{
					...
					if (!(textDiscussion.isEmpty()))
					{
						...
						textDocuments.add(
								createTextDocument(
										headings.get(index),
										textDiscussion,
										(index), bookProperties.documentType.get(1), "", bookProperties)
								);
					...
					if (!(text.isEmpty()))
					{
						index++;
						text = cleanText(text);
						textDocuments.add(createTextDocument(headings.get(index - 1), text, index, bookProperties.documentType.get(0), createSectionText(text), bookProperties));
						text = "";
					}
				}
				...
				{
					if (
							!line.matches("\\d+([%°])*|\\d+\\s\\d+|\\d+\\.\\d|\\+(\\s\\d+)*")
									&&
									(!line.startsWith("M. Thiel, W. Bernauer, M. Zürcher Schüpfer, M. Schmid (Hrsg.), Fallbeispiele Augenheilkunde, "))
									&&
									(!line.startsWith("DOI 10.1007"))
									&&
									(!line.matches("M\\. Zürcher Schüpfer, W\\. Dedes"))
									&&
									...
									&&
									(!line.matches("\\s\\.\\sAbb\\.\\s\\d+\\.\\d+\\s[a-zA-Zöäüß0-9().„“»«,:;-]+(\\s+[a-zA-Zöäüß0-9().„“»«,:;-]+)*\\s*"))
									&&
									(!line.startsWith(" . Abb. "))
									&&
									(!line.startsWith(" Granulomatöse Konjunktivitis"))
									&&
									...
									&&
									(!line.equals("K. Landau"))
									&&
									(!line.equals(" Traumatisches Maculaloch"))
									&&
									...
									&&
									(!line.equals(" Stauungspapille: Wo liegt die Ursache?"))
									&&
									(!line.equals("N. Lansel"))
									&&
									...
									&&
									(!line.equals("der Bindehaut"))
									&&
									(!line.startsWith("Substantia propria"))
									&&
									(!line.startsWith("W. Bernauer, J.K. Lacoste"))
									&&
									(!line.equals(" Kataraktoperation als Drucksenkung bei "))
									&&
									(!line.equals("Kapselhäutchen und Kapselhäutchenglaukom?"))
									&&
									(!line.equals(""))
					)
					{
				...
				if (readDiscussionText)
				{
					if (
							(!(line.matches("\\d+(\\s+\\d+)?")))
									...
									(!(line.startsWith("DOI 10.1007")))
									&&
									(!(line.startsWith(" z Diskussion")))
									&&
									(!(line.equals("Diskussion")))
									&&
									(!(line.startsWith("M. Thiel")))
									&&
									(!(line.startsWith("M.A. Thiel")))
									&&
									(!(line.matches("Kapitel \\d+\u2002 •.*")))
									&&
									(!(line.startsWith(" Fataler Tumor des")))
									&&
	...
	private static String cleanDiscussionText(String text, int index)
	{
	...
	private static TextDocument createTextDocument(String heading, String text, int index, String type, String textSection, BookProperties bookProperties)
	{
		if (type.equals(bookProperties.documentType.get(0)))
		{
			index = ((index * 2) - 1);
		} else {
			index = index * 2;
		...
		textDocument.topics.addAll(bookProperties.getTopics());
