...
public class NormalizationUtils {
	...
	private static final Pattern HYPHEN_NEWLINE_LOWER
		= Pattern.compile("-\\s?\n(?=\\p{javaLowerCase})", Pattern.MULTILINE);
	
	private static final Pattern HYPHEN_NEWLINE_UPPER
		= Pattern.compile("-\\s?\n(?=[^\\p{javaLowerCase}])", Pattern.MULTILINE);
	...
	{
		text = Arrays.stream(text.split("\\r?\\n")).map(NormalizationUtils::normalizeLine).collect(Collectors.joining(""));
